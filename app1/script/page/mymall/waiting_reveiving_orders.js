define(function(require, exports, module){
	...
	//待付款
	function loadWaitReceive(){
		appFrame.showShadowCover(true,null,'waitingReveivWrap');
		server.getNonReceiveOrderData(getWaitReceive,datas);
	}
	function getWaitReceive(data){
		if(data.result==1){
				...
				appFrame.closeShadowCover();
					...
					if( data.orders.length<1){
						var html = '<div>'+
						    ...
						    '<p class="No_payment_word txt_1"><span class="inter_lang_font" langkey="lang_msg_no_unreceipted">' + getLangVal("lang_msg_no_unreceipted") + '</span></p>'+
						'</div>';
						$("#waitingReveiv").html(html);
					}else{
						$("#waitingReveiv").html(doTtmpl(data));
					}
					appFrame.closeShadowCover();
			}
			
			if($("#waitingReveiv").attr("scrollHeight") > $("#waitingReveivWrap").height()){
				if($("#pullUp").length <= 0){
					...
					$(loadingDataString).appendTo($("#waitingReveiv"));
				}
			}
			...
			if(orderListScroll){
				...
				orderListScroll = new iScroll.iScroll("waitingReveivWrap",{
					useTransition: true,
					onRefresh: function () {

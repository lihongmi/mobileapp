define(function(require, exports, module){
	...
	function loadCompleted(){
		appFrame.showShadowCover(true,null,'completedWrap');
		server.getFinishOrderData(getCompleted,datas);
	}
	function getCompleted(data){
		if(data.result==1){
				...
				appFrame.closeShadowCover();
					...
					if( data.orders.length<1){
						var html = '<div>'+
						    ...
						    '<p class="No_payment_word txt_1"><span class="inter_lang_font" langkey="lang_msg_no_completed">' + getLangVal("lang_msg_no_completed") + '</span></p>'+
						'</div>';
						$("#completedOrders").html(html);
					}else{
						$("#completedOrders").html(doTtmpl(data));
					}
					appFrame.closeShadowCover();
			}
			
			if($("#completedOrders").attr("scrollHeight") > $("#completedWrap").height()){
				if($("#pullUp").length <= 0){
					...
					$(loadingDataString).appendTo($("#completedOrders"));
				}
			}
			...
			if(orderListScroll){
				...
				orderListScroll = new iScroll.iScroll("completedWrap",{
					useTransition: true,
					onRefresh: function () {

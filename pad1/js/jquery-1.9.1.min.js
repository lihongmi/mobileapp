 ...
 */
	console.log(jQuery);
(function(a){a.tools=a.tools||{version:'@VERSION'};a.queryToJson=function(b,c){var d=a.trim(b).split("&");var e={};var f=function(h){if(c){return decodeURIComponent(h)}else{return h}};for(var g=0,len=d.length;g<len;g++){if(d[g]){var h=d[g].split("=");var i=h[0];var j=h[1];if(h.length<2){j=i;i='$nullName'};if(!e[i]){e[i]=f(j)}else{if(a.isArray(e[i])!=true){e[i]=[e[i]]};e[i].push(f(j))}}};return e}})(jQuery);
/**
 * jsonToQuery
 */
(function(a){a.tools=a.tools||{version:'@VERSION'};var b=function(c,d){c=c==null?'':c;c=a.trim(c.toString());if(d){return encodeURIComponent(c)}else{return c}};a.jsonToQuery=function(c,d){var e=[];if(typeof c=="object"){for(var f in c){if(f==='$nullName'){e=e.concat(c[f]);continue};if(c[f]instanceof Array){for(var g=0,len=c[f].length;g<len;g++){e.push(f+"="+b(c[f][g],d))}}else{if(typeof c[f]!='function'){e.push(f+"="+b(c[f],d))}}}};if(e.length){return e.join("&")}else{return""}}})(jQuery);
/**
 * htmlToJson
 */
(function(a){a.tools=a.tools||{version:'@VERSION'};var b=function(c){var d=c.getAttribute("name");var e=c.getAttribute("type");var f=c.tagName;var g={"name":d,"value":""};var h=function(i){if(i===false){g=false}else if(!g["value"]){g["value"]=a.trim((i||""))}else{g["value"]=[a.trim((i||""))].concat(g["value"])}};if(!c.disabled&&d){switch(f){case"INPUT":if(e=="radio"||e=="checkbox"){if(c.checked){h(c.value)}else{h(false)}}else if(e=="reset"||e=="submit"||e=="image"){h(false)}else{h(c.value)};break;case"SELECT":if(c.multiple){var i=c.options;for(var j=0,len=i.length;j<len;j++){if(i[j].selected){h(i[j].value)}}}else{h(c.value)};break;case"TEXTAREA":h(c.value||c.getAttribute("value")||false);break;case"BUTTON":default:h(c.value||c.getAttribute("value")||c.innerHTML||false)}}else{return false};return g};a.fn.htmlToJson=function(c,d){var e={};c=c||["INPUT","TEXTAREA","BUTTON","SELECT"];var f=this[0];if(!f||!c){return false};var g=b;for(var h=0,len=c.length;h<len;h++){var i=f.getElementsByTagName(c[h]);for(var j=0,lenTag=i.length;j<lenTag;j++){var k=g(i[j]);if(!k||(d&&(k.value===''))){continue};if(typeof e[k.name] !== "undefined"){if(e[k.name]instanceof Array){e[k.name]=e[k.name].concat(k.value)}else{e[k.name]=[e[k.name]].concat(k.value)}}else{e[k.name]=k.value}}};return e}})(jQuery);

var LIGHT = false;
(function($){
	$.delegate = function(selector, evtType, callback, parent){
		$(parent || document.body).delegate("[evt-type="+selector+"]", evtType, function(evt){
			evt.data = $.queryToJson($(this).attr("evt-data")) || {};
...
(function($){
	$.fn.jsNode = function(selector){
		return $(this).find('[js-node='+selector+']');
	};
})(jQuery);
(function($){$.parseParam = function(oSource, oParams, isown){ var key, obj = {}; oParams = oParams || {}; for (key in oSource) { obj[key] = oSource[key]; if (oParams[key] != null) { if (isown) { if (oSource.hasOwnProperty[key]) { obj[key] = oParams[key]; } } else { obj[key] = oParams[key]; } } } return obj; };})(jQuery);
(function($){
	var checkOriginObj = function (obj) {
		...
		if ($.inArray((typeof obj), ['number', 'string', 'function'])) {
			return true;
		}
		if ($.isArray(obj)) {
			return true;
	...
	$.tools.merge = function (origin, cover, opts) {
		var conf = $.parseParam({
			'isDeep': false
		}, opts);
...
//	window.$LANG || (window.$LANG = {});
	var _templet = function (template, data) {
	...
	var _lang = function (template, data) {
	...
	console.log(222);
	$.language = function (temp, data) {
//		var _LANG = (Base && Base.DataBase && Base.DataBase.english == '1') ? $LANG : {};
		var _LANG = localStorage.getItem('en') == '1' ? $LANG : {};
//		console.log('lang:', _LANG, Base.DataBase.english, Base.DataBase.english == '1')
		var str = _lang(temp, _LANG);
		str = str.replace(/\\}/ig, '}');
		if (data) {
